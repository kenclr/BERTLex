...
# added MetadataField
from allennlp.data.fields import ArrayField, LabelField, SpanField, TextField, MetadataField
import numpy as np
...
class ClresConlluReader(DatasetReader):
    ...
    #added metas
    def text_to_instance(
        self, metas: Dict[str, Any], tokens: List[str], span_start: int, span_end: int, label: str, embeddings: np.ndarray = None
    ) -> Instance:
        meta_field = MetadataField(metas) #this added
        tokens = [Token(t) for t in tokens]
        # The text of the sentence in which the word sense appears
        text_field = TextField(tokens, self.token_indexers)
        # The word sense-annotated span, always of length 1 in ontonotes
        ...
        # like above but without the sense number
        lemma_field = LabelField(lemma_from_label(label), label_namespace="lemma_labels")
        fields = {"text": text_field, "label_span": lemma_span_field, "label": label_field, "lemma": lemma_field, "metas": meta_field}
        if self.embedding_predictor:
            fields["span_embeddings"] = ArrayField(embeddings[span_start : span_end + 1, :])
    ...
    def _read(self, file_path: str) -> Iterable[Instance]:
        ...
        for tokenlist in tokenlists:
            ...
            metas = tokenlist.metadata # added here
            ...
            if self.embedding_predictor:
                ...
                embeddings = None
            yield self.text_to_instance(metas, tokens, prep_index, prep_index, lemma + "_" + label, embeddings=embeddings)
